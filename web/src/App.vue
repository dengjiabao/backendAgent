...
<template>
  <main class="shell">
    ...
    <section v-else-if="activeView === 'approvals'" class="panel empty"><span class="number">02</span><h2>审批中心</h2><p>待处理提案会在这里聚合。当前演示可从对话工作台生成商品更新提案。</p><button @click="activeView = 'chat'">返回对话</button></section>
    <section v-else-if="activeView === 'knowledge'" class="panel empty"><span class="number">03</span><h2>知识库</h2><p>上传制度、源码和运营手册，转换后生成可引用证据。</p><label class="dropzone">选择 Markdown 文件<input type="file" accept=".md,.txt" hidden /></label></section>
