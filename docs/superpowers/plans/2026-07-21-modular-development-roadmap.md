...
- [x] 将工具提案、审批决定和真实执行拆为独立应用服务。
- [x] 实现编辑后 JSON Schema/权限重新校验。
- [x] 实现审批超时自动拒绝和写操作幂等键。
- [x] 对提案、决定、执行、失败写结构化审计事件。
- [x] 退款、删除、角色权限变更保持默认禁止。
- [x] 通过状态机、并发和重复决定测试后创建 `docs/modules/05-approval-execution/README.md`。
...
- [x] 保持 API Client、视图和状态管理分层，避免页面直接拼装后端 DTO。
- [x] 完善对话、审批、知识库、运行轨迹、评估和配置页面。
- [x] 增加构建级交互验证，后续可在运行环境具备时补充 Playwright E2E。
- [x] 通过构建后创建 `docs/modules/09-console/README.md`。
